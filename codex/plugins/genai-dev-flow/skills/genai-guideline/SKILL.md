...
- **A failed acceptance run routes by classification, and there are exactly two destinations.** A
  ...
  graph can act on; everything else the run gets wrong is its own to fix. The exemption has a floor:
  a delegation that routes nowhere, or reaches a target already holding the same work, costs the
  whole remaining patience, since "the work moved to somebody" is then false.
- **Four steps carry the byte-identical check** — `genai.spec`, `genai.implement`, `genai.e2e-author`
  and `genai.e2e`. Count them with `grep -l 'checker:.*signature_changed'` over the node definitions
  rather than trusting this line; the number written here has been wrong before. Elsewhere the gates
  assert an end state doing nothing cannot satisfy, so a delta check adds nothing and makes rework
  fragile — a stalled signature drains the patience budget at once. `genai.e2e` was outside that
  group until a run classified from the **previous** attempt's report, still on disk because its
  executor died first: an end state is no defence when the artifact carrying it is not this
  attempt's.
- **An entry rule that its own step invalidates has to say so, with `when`.** By default a rule
  ...
  every rework and charge a patience point each time, until the round suspended on a condition the
  step's own success had made permanently false.
...
**Two shapes, and exactly one of them per project**: `url` for anything that listens, `command` for
everything that never will — a CLI, a library, a batch job. That second shape is not a convenience.
`genai.merge` premises on `genai.e2e`, which refuses to **start** without an identified app, so
before it existed a project with no HTTP surface could complete every other step of a round and
never merge. A project with more than one face declares `targets` instead, and every one of them has
to be identified. The exact fields, the several-faces rules and what makes a marker hold up are in
[references/e2e-identity.md](references/e2e-identity.md).
...
goes to a person. Refusing to start still beats failing afterwards — a run against whatever else was
listening produces a report about the wrong system, and no later step can tell.

**It is not free, though, and this is the rule most likely to meet someone who assumes it is.** An
entry refusal spends a patience point, so dispatching repeatedly to see whether the app has come up
walks a round into a suspension. Ask with `fsx next --check-ready`, which evaluates the same rule
and writes nothing.
...
leave it absent, and the state that follows is the intended one — `genai.e2e` refuses entry with
`config_missing`, which names exactly what is needed. It is still a refusal that costs a patience
point, so answer it before dispatching that step rather than through it.
...
**The target never says pass or fail, and its exit code is ignored.** It prints one line per module
beginning `genai-metrics: `, each carrying a JSON object of test counts and coverage fractions; the
judgement lives in `.flow/genai/`, which ships with the step definitions. **That is what makes the
gate a gate** — a project that decides its own verdict has none, and a round that can edit the
arithmetic is not being measured by it. Three rules are universal and live in `check.mjs` (no test
may fail, at least one ran, at most a tenth skipped); the coverage floors cannot be, and live in the
project's own `tools/genai/thresholds.json`, keyed per module and per dimension.

A setup that is broken rather than failing says so in its own labels — `metrics_missing`,
`metrics_unreadable`, `thresholds_missing` — and all three reject as setup problems, because none of
those files are the round's to fix.

**The exact line format, the field table, the make recipe that does not swallow its own output, and
how the floors are written are in [references/metrics-target.md](references/metrics-target.md).**
Read it before writing that target or moving a floor; the two rules below are the ones that apply
whether or not you are the person writing it.
### Who may change it
