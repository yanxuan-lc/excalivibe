...
"${CODEX_PLUGIN_ROOT}/skills/notify-user/scripts/notify.sh" --doctor
```
...
```bash
"${CODEX_PLUGIN_ROOT}/skills/notify-user/scripts/notify.sh" --init     # writes the config, both lines live
"${CODEX_PLUGIN_ROOT}/skills/notify-user/scripts/notify.sh" --doctor   # prove it, with a real banner
```
