...
- **Fixed canonical top-level dirs** — use these exact names when the concern exists, don't invent synonyms: `protocol/` (protocol / API contract), `database/` (database / schema / DDL), `nacos/` (config center / config delivery). They recur in almost every backend project; consistent names let a reader navigate an unfamiliar repo. A `database/` is always `database/`, never `schema/` or `db/`.
...
- `v1.0/` corresponds to `/v1`. A **breaking** change (removed/renamed field, changed semantics, auth change) starts a new major dir: `v2.0/`, and both can coexist during a migration window.
- **A backward-compatible revision is an edit to the current directory, not a new one.** Adding an optional parameter or a new endpoint to `/v1` does not give consumers a second contract to choose between — there is one live `/v1`, and `v1.0/` is what describes it. Record the addition in place and note when it landed.

  A new minor directory (`v1.1/`) is warranted only when both revisions are **simultaneously live and separately addressable** — a versioned media type, a pinned header, a parallel deployment. If a reader cannot pick one, do not make them read two.

  [MUST] **The no-duplication rule decides ties.** `v1.0/` and `v1.1/` differing by one optional parameter means the same wire contract is described twice, and the two copies start drifting the day after they are written.
- **Docs before code**: the version dir and its README update *before or with* the code, never after. The contract doc is the agreement; code implements it. Shipping code first means the doc describes a past that no longer exists.
...
- **Copying the proposal.** Whatever recorded the change already is the narrative record. `tech/` is the steady-state distillation, not a second copy of the proposal.
