...
"Confirm" needs a rule you can actually apply, or it degrades into a glance at the connection
string. Take the first of these the project offers, and say in the report which one you used:

1. **An explicit allowlist** — `E2E_ALLOWED_DATABASES` / a `test-db` field in the project's e2e
   config. Best, because someone stated the intent rather than you inferring it.
2. **A name convention** — the database name matches `/(^|_)(test|staging|e2e|dev)(_|$)/i`.
   Match on the **database name**, not the host: `app_e2e` on a shared host is fine, `app` on a
   host called `staging-db` is not — hostnames get reused and renamed, and the name of the
   database is what the write lands in.
3. **A credential that cannot do harm** — a read-only account. Also good, and worth asking for.

None of the three available is a **blocked precondition**, not a judgement call to make on the
spot. "It's probably the test one" has the same shape as every production incident that starts
with a query. The cost of stopping is a message; the cost of being wrong is unbounded and not
yours to accept.
