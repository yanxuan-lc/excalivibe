...
- Use imperative mood for functions: "Fetch the user profile" not "This function fetches the user profile" or "Fetches the user profile".
  **Except where the language has its own doc convention** — Go doc comments open with the
  declaration name in the third person (`// FetchUser retrieves …`). The language-specific guide wins.
