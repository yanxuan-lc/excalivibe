...
> **状态：2.x，已替代并移除 1.x。** 插件名 `gen-ai-development`，当前版本 `2.3.0`，skill 命名空间为 `gen-ai-development:*`。Claude 与 Codex 两端走同一主流程；仅 harness primitives 各自最优（见下文 Agents / 兜底安全网两节）。
