...
description: "DevOps conventions for a repository — one self-documenting task-runner front door (Makefile / justfile / npm scripts / Taskfile), docker-compose for local dev middleware (DB / MQ / cache), and the multi-stage non-root China-buildable Dockerfile. Use when creating or editing a task runner, adding build/test/lint/fmt/run entry points, writing a docker-compose for local deps or a Dockerfile, or answering \"how do I run/build X in this repo\". Trigger even without \"Makefile\"/\"guideline\": anything touching a task-runner file, dev compose file, Dockerfile, or repo-side CI entry points (the task-runner commands CI invokes — not CI-platform YAML syntax) consults this first. Service-side config/metrics/logging integration belongs to middleware-guideline."
---
