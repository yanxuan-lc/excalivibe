...
description: Conventions for a service's integration with platform middleware and monitoring — the config center (Nacos two-layer bootstrap/business config, `<APP>_PROFILE`-driven namespace isolation, fast-fail with no silent fallback) and the monitoring surface every backend service must expose (`/healthz`, `/readyz`, Prometheus `/metrics`). Use it whenever connecting a service to a config center, reading runtime config at startup, adding profile/environment selection or config hot-reload, scaffolding a new backend service, designing its API/interface surface, or adding health checks / metrics endpoints. Trigger it even when the user just says "接 Nacos", "读配置", "配置中心", "健康检查", "加个指标", "接监控", or "设计服务端接口" — if the work shapes a backend service's endpoints or touches how it fetches/reloads config or exposes health and metrics, consult this skill first and self-review against it. Local docker-compose setup belongs to the companion devops-guideline skill; this one owns the service side.
---
