...
description: >-
  The gen-ai-development research orchestration pipeline — how the main agent runs a research task end to end: Socratic clarification with the user, conclusion confirmation, a research plan that routes subtasks to the deep-research skill / researcher agents / inline answers, parallel agent spawns, and synthesis into docs/research/<datetime>-<topic>/ (REPORT.md + PROPOSAL.md). Use this skill whenever the user wants something researched, investigated, compared, or feasibility-checked — "调研一下X", "对比A和B", "这个方案可行吗", "帮我看看这些资料" — and when running the research phase of the dev-pipeline. All user interaction stays in the main agent here; researcher agents are pure executors that cannot ask the user anything.
---
...
agent (`codex/agents/researcher.toml`) is a **pure execution unit** — it receives a
scoped question, probes, and returns structured findings; it never talks to the
...
- **Quick lookup** (one concept, one library question, one known source) → answer
  inline or spawn a single researcher agent; skip Steps a–b entirely.
...
Clarify the request by **asking the user**, Socratic style: one purposeful
...
| # | 子问题 | 执行方 | 方法 | 模型 | 预期产出 |
|---|--------|--------|------|------|----------|
| S-a | 业界方案盘点 | deep-research 技能（主 agent 直接运行） | fan-out web | — | 带引用的方案清单 |
...
- **Broad multi-source fact-finding / wide solution scans** → the **`deep-research`
  skill**, run BY THE MAIN AGENT directly (the main agent can run multi-step skill
  workflows; a spawned agent often can't — this is why Tier-3 no longer lives inside
  researcher). `deep-research` is an external skill, not shipped with this plugin —
  if it isn't available, degrade explicitly: split the broad question into narrower
  subtopics and spawn them as parallel researcher investigations (each using web
  search / context7 within its scope), then note the degradation in REPORT.md.
- **Hands-on probing or a focused subtopic** → spawn a **`researcher` agent**, naming
  the method skills it should use (research-source-code / research-data-source /
  research-api / context7 / web search).
- **Light conceptual questions** → answer inline; don't spawn an agent for what you
  can settle yourself.
- Pick the **model per spawn** when you request the agent: judgment-heavy subtopics →
  the stronger model; mechanical probing → a lighter one. Default: inherit the
  session model (don't hardcode a model name).

For plans with more than a couple of subtasks, show the plan to the user briefly
before spawning — it's their token budget.

## Step d — Dispatch (parallel)

Spawn independent subtasks **in one request** so they run in parallel. Each spawn
follows the task protocol in [references/protocols.md](references/protocols.md):
scoped question + the Step-b digest + suggested methods + depth + the output schema.
Researchers return findings **inline** (they do not write `docs/research/` — parallel
writers conflict and each holds only a partial view).

## Step e — Synthesis, Follow-ups, and the Loop

1. Read every result. **Judgment stays home**: you weigh conflicting findings,
   you own the conclusion — never paste a spawned agent's output through unread.
2. Collect all `open_questions` from the results and ask the user in one batch.
   This replaces mid-research interruptions — researchers park questions instead of
   blocking on them.
...
4. **Synthesize via one final researcher spawn** (`mode: synthesize`, see
   protocols.md): input = all findings + the Step-b digest + the user's follow-up
   answers; output = REPORT.md + PROPOSAL.md written to
   `docs/research/<YYYY-MM-DD_HH-mm-ss>-<topic>/` (this spawn is the one that
