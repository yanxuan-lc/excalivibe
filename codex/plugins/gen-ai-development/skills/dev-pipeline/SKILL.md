...
gen-ai-development agents, which gates block which transitions, and where the
pipeline's state lives. The capabilities themselves (researcher, planner, developer,
quality-assurance, arch-reviewer, code-reviewer, e2e-runner — defined as
`codex/agents/*.toml` — and the guideline skills) do the work; this skill is the
wiring between them. Spawn an agent by asking Codex for it explicitly (e.g. "spawn
...
researcher ──(opt)──▶ app-ux-design ──(opt)──▶ planner ──▶ arch-reviewer ──(opt, 裁量)──▶
        propose 产物: openspec/changes/<id>/（含人审文档 REVIEW.md）   闭环: planner 改 spec + 重生成 REVIEW.md

  ┌─ developer (apply, TDD: 业务逻辑 + 单测 + lint) ─────┐
  │                                                      ├──▶  e2e-runner ∥ code-reviewer ──▶ merge → dev ──▶ archive + docs
  └─ quality-assurance (e2e 测试代码 + manifest) ─────────┘        (read-only, 同一请求并行 spawn)        (vcs-workflow)     (docs-guideline)
```
...
- **developer ∥ quality-assurance** start from the same confirmed spec and can be
  spawned in the same request — but QA's work is **two-phase**, because a
  spawned agent runs once and returns; it cannot wait for developer:
  ...
  - **Phase 2** (after developer delivers AND the main agent boots the app —
    ...
    (send it a follow-up message) with "app is running at <url>, finalize" — it
    fixes selectors against the real DOM, runs its suite to green, and finalizes
    the manifest.
...
- **e2e-runner ∥ code-reviewer** are both read-only — always spawn them in one
  request. Pass code-reviewer `mode: incremental` explicitly. Before spawning
...
- **Fix loop (convergence protocol)**: e2e failures classified "product bug" route
  ...
  2. Re-spawn in one request: code-reviewer (fix diff only, **pass it the
...
leverage: **after arch-review closes, before spawning developer ∥ QA**. Present
...
- Record the confirmation as the `spec-confirm` row in PIPELINE.md. **developer and
  quality-assurance are not spawned while it is unchecked.**
...
- **Parallel-first**: independent agent spawns go in one request
  (developer ∥ QA; e2e-runner ∥ code-reviewer).
- **Judgment stays home**: a spawned agent's output is an input to the main agent's
  decision, not a verdict to forward blindly. Read the key artifacts yourself before
  acting.
- **Spawned agents can't talk to the user**: if an agent returns questions instead of
