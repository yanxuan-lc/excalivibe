...
1. Manifest missing or run command broken → ALL scenarios agent-driven
   (graceful-browser; Codex native browser first — priority order's authority is
   the graceful-browser skill itself). Report this downgrade explicitly.
...
- **`connectOverCDP`**: scripts honor an env var (e.g. `PW_CDP_ENDPOINT`,
  ...
  profile, and the Codex native browser exposes no scriptable surface for this.
