...
- **Continuation etiquette.** When resuming/continuing an agent (a stalled run, a
  missing artifact, relayed answers): FIRST verify the agent ↔ role mapping — parallel
  spawns are identified in call order, and a resume message sent to the wrong role costs
  a full round-trip (measured: a "write your missing CHECKLIST.md" message landed on the
  e2e-runner). THEN self-identify and restate context inside the message ("I am the
  dispatching controller; your run ended without X on disk; continue from Y"). A resumed
  agent that treats an unattributed instruction as injected content and re-derives
  everything from scratch is behaving CORRECTLY per prompt-injection hygiene — but it
  costs a full re-verification round (measured: ~56 min). Give it the provenance it
  needs to trust the message.
