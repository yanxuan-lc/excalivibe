...
| `design-spec` | `openspec/changes/<id>/` (4 contracts; module design via glossary terms) | agent (`planner`) |
...
| `implement` | commits + unit tests | agent (`developer`, `tdd`) |
| `e2e-author` | e2e tests + `e2e-manifest.md` (from spec only) | agent (`e2e-author`) |
| `e2e-run` | `e2e-report.md` (read-only) | agent (`e2e-runner`, `e2e-test`) |
| `code-review` | `CHECKLIST.md` (two-verdict, read-only) | agent (`code-reviewer`) |
...
| `publish` | npm / Nexus release (the single human consent gate) | agent (`release-coordinator`) + ⟦consent⟧ |
