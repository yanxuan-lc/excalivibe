...
`Hero` `Footer` `Section` `Callout` `Card`(badge/badgeTone) `Columns` `Toggle` `Steps`/`Step` `Stats`/`Stat` `Fields`/`Field` `Scenario`/`When`/`And`/`Then` `Grid`/`Item`(filterable/facets/tags) `Math` `Code` `Badge` `Diagram`(占位)。
...
图引擎（Mermaid / D2 / 自研分层渲染器）**方案待定**，`<Diagram>` 暂为占位。

## 硬约束

- **自包含、零外链**：产物不引任何 CDN；第三方库（KaTeX 等）vendored 到 `assets/vendor/`，React/MDX/Tailwind 是**构建期**依赖不进产物。
