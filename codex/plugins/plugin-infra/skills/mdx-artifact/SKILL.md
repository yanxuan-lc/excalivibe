---
name: mdx-artifact
description: 将报告、方案、设计总览、看板、说明、Markdown、纯文本或分析结论写成组件化 MDX，并用全局 CLI `mdxv` 起本地预览给人阅读。当用户要制作给人阅读的文档，或提出“美化文档”“整理成好看的文档”“做成网页”“生成可交付产物”时使用，即使没有明说 MDX 或组件；不要改为手写整篇 HTML 或只交付朴素 Markdown。不用于需要后端或实时数据的 Web 应用、多人在线协作文档、电子表格或数据库应用。
---
...
它只回答「能不能编译」。通过意味着**编译阶段没有报错**，不保证预览实际加载成功，更不意味着
内容或呈现是对的：
...
- **预览是常驻进程**：在 Codex unified exec 中直接运行 `mdxv doc.mdx --no-open`，**不要加 `&`**；
  让 exec 短暂 yield 并保留返回的 session ID，从输出取得 `http://localhost:4321/?doc=…`。交付前
  用浏览器打开该 URL，或以 `curl --fail --silent --show-error '<URL>' >/dev/null` 请求一次，确认实际
  加载不是 4xx/5xx；失败就修文档，别交付。用完通过该 session 发送 Ctrl-C 主动停掉。
