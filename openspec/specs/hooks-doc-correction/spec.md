# hooks-doc-correction Specification

## Purpose

Corrects the framing of the Codex `hooks` prohibition across three project documents (`CLAUDE.md`, `AGENTS.md`, `codex/ADAPTING-FROM-CLAUDE.md`) so the bare prohibition gains the precise three-layer nuance: (a) the plugin manifest `hooks` field is rejected by `validate_plugin.py`; (b) the Codex runtime hooks capability exists via `config.toml`; (c) non-interactive exec firing is unverified / trust-gated; (d) v1 does not depend on hooks. The functional prohibition (do NOT add `hooks` to `.codex-plugin/plugin.json`) is preserved in all three files.

## Requirements

### Requirement: CLAUDE.md hooks framing is precise

The `CLAUDE.md` (project-root) `hooks` prohibition line (~L16) SHALL be edited to gain the precise three-layer framing described below. The line currently reads (verbatim):
...
The `AGENTS.md` (project-root, shared across agents) `plugin.json` hooks line (~L68) SHALL gain the two missing nuance clauses described below. The line currently reads (verbatim):
...
The `plugins/<p>/hooks/` row in the §1 directory/manifest mapping table of `codex/ADAPTING-FROM-CLAUDE.md` (~L19) SHALL be updated to the three-layer framing described below. The row currently reads (verbatim):
