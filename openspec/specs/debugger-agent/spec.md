...
Defines the `debugger` subagent shipped by the `gen-ai-development` plugin: a Claude-end agent definition (`agents/debugger.md`) and a Codex-end TOML subagent (`codex/agents/debugger.toml`). The agent embodies a hypothesis-first, minimal-fix, mandatory-cleanup debugging persona that defers browser symptoms to `graceful-browser` and hands off to `tdd` after a confirmed fix. Includes the install-time copy procedure for the Codex TOML and the CLAUDE.md subagent-table registration.
