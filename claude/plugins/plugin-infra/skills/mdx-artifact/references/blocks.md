...
写 `.mdx` 时的组件参考，对齐 **mdx-viewer ≥ 0.2.0**（渲染器是全局 CLI `mdxv`，权威源是该包本身；`mdxv demo` 可直接看到每个组件的渲染效果）。
...
| `toc` | `true` 右侧悬浮目录；**≤1700px 视口自动隐藏**（笔记本上多数看不到，别当作承诺） |
...
- **全屏查看**：每张图右上角有放大按钮（hover 显现，触屏常显），点开进入全屏查看器——**滚轮缩放（跟随光标）、拖拽平移、放大/缩小/适应窗口工具条、Esc 或点背景关闭**；缩放走 SVG 固有尺寸而非 CSS transform，放到多大都清晰。无需作者做任何事，三种引擎的图都自动具备。
