...
- **`coding-guideline`** — your test code follows it and the project's lint config; run
...
- **Declare who verifies the DB write.** Each *mapped* scenario carries a `db-assert`
  value: `suite` (your test code itself performs the DB verification per the `e2e-test`
  conventions) or `runner` (left to e2e-runner's own scoped SELECTs). Declare `suite`
  only when the test genuinely asserts the write — e2e-runner samples suite-asserted
  scenarios instead of re-verifying each one, so a false `suite` weakens the acceptance
  evidence. Agent-driven scenarios are always `runner`.
...
|----------|---------------|-------------------------------------------------|-----------|
| S1       | mapped        | e2e/registration.spec.ts → 'S1: 新用户注册成功' | suite     |
| S2       | agent-driven  | live-driven by e2e-runner (no scripted seam)    | runner    |
...
- **Different model family on gated lanes.** On a gated lane the caller dispatches you
  under a different model family than developer where the harness permits (else it records
  the gap); either way you do not assume you share developer's reading of the spec. (Independent *context* is not independent
