...
large timeout (up to 600000 ms); split anything longer into batches. Background a
command ONLY to overlap it with other useful work: write its REAL exit code to the log
...
**Verification economy:** one verify command per iteration, scoped to the change (the
repo's `check-diff` target or run-level test filters) — never chain redundant passes
(typecheck → test → lint) over the same tree in an inner loop. What the test command
subsumes differs by toolchain (in compiled toolchains it already type-checks; with
Vitest/Jest the typecheck is a separate task-boundary pass) — the tdd toolchain guide's
Verification Discipline section is authoritative. The full-scope suite + lint sweep runs
once, at the merge gate, and its result is recorded (command, real exit code, commit).
