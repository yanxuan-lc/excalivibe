...
> agent definition (`agents/e2e-runner.md`) — foreground with explicit timeout, never
> stop-and-wait on a background command. The rules live there, not here, to avoid drift.
...
1. **Pick the framework via the `graceful-browser` skill** (from the `plugin-infra`
   plugin): claude --chrome first, then the chrome-devtools MCP, then the Playwright
