...
1. **MCP data tool (preferred).** If a database/warehouse MCP server is connected this session, use its query interface — access is already scoped and you avoid handling raw credentials. Use `ToolSearch` to find it (e.g. query for the engine name) if it's deferred.
