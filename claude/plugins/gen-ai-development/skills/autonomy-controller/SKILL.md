...
track, dispatches the subagents that do the work, and reads/writes the durable state.
**It implements no capability itself — it arranges.** Every capability lives in a
functional skill (`tdd`, `grill`, `security-scan`, …) or a subagent (`developer`,
...
- **subagent dispatch** — when 専人専事 / context isolation matters (everything in the
...
The node is the orchestration concept; Tier-1 skills and Tier-2 subagents are its two
implementation vehicles. This split is what keeps the toolkit portable and lean.

## Step 5 — Dispatch the subagents
...
- **The QA boot-boundary is a real main-agent seam.** `e2e-author` works two-phase: a
  ...
  main agent boots the app — a continuation that finalizes against the real DOM. A
  subagent runs once and returns; it cannot wait for another.
...
- **Verifier tier follows the lane**: spot-check / light lanes dispatch `code-reviewer`
  ...
  requirement are reserved for human-gated lanes and the post-merge audit. Agent-frontmatter models
