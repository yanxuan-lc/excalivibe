...
一个面向 Claude Code 的**交互式 UI/UX 设计 skill**：把一句模糊的需求（"帮我设计一个
...
用户通过两种方式给反馈：直接和 Claude Code 对话，或在浏览器里用 DevTools 风格的 Inspect
面板点选元素、改文案 / 改样式 / 留批注。
...
你不用手动跑任何命令。在 Claude Code 里直接用自然语言提需求即可，常见触发说法：
...
Claude Code 会：先和你对齐需求与模式 → 调 ui-ux-pro-max 敲定设计系统 → 在
...
```
.
├── SKILL.md              # Claude Code 读取的工作流指令（先读这个）
