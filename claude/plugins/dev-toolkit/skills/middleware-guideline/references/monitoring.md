...
**Where the instrumentation goes in the chain decides what it can see.** Wrap the whole handler,
outermost — before auth, before rate limiting:

```go
// outermost → innermost:  instrument( auth( rateLimit( mux ) ) )
srv.Handler = instrument(reqDur)(authMiddleware(rateLimit(mux)))
```

Put it inside auth instead and every rejected request returns before reaching it: **401s and
429s stop existing as far as your metrics are concerned.** The dashboard then looks healthy
during exactly the incidents you built it for — a credential rollout that is failing, a client
hammering into a rate limit. An error rate that cannot go up is not a low error rate.

The same rule is why the `code` label is there at all: it is only meaningful if the failures
