...
|----------|-----------|---------|
| TypeScript / React | `<module>.test.ts(x)` (under `tests/`, mirroring `src/`) | `tests/services/auth.test.ts` |
| JavaScript | `<module>.test.mjs` (under `tests/`, mirroring `src/`) | `tests/services/auth.test.mjs` |
...
**Where the file goes is the `tdd` skill's call, not this table's.** This table names files; the
`tdd` toolchain guides own the layout, and they mirror `src/` under `tests/`. The two must agree —
an agent under both skills at once gets no useful answer from a directory it has to pick between.
Deviations that the language's own tooling forces (Go's same-package `_test.go`, Dart's `test/`,
Cargo's `tests/`) are the tooling's rule, not a third opinion.
