...
# ---------- build stage ----------
ARG GO_VERSION=1.23
ARG BASE_REGISTRY=docker.io            # override to a domestic mirror; see "Pulling the base image"
FROM ${BASE_REGISTRY}/library/golang:${GO_VERSION}-alpine AS builder
...
# `go.sum*` — the glob matters: a module with no external dependencies never generates a
# go.sum, and a bare `COPY go.mod go.sum ./` fails the build on exactly that project.
COPY go.mod go.sum* ./
...
# ---------- runtime stage ----------
ARG ALPINE_VERSION=3.20                # symmetric with GO_VERSION — pin both, override both
...
### Pulling the base image — the half that the table below does not cover

Everything in the table is about packages fetched *during* the build. **The base image itself is pulled before any of it runs, and it is usually the slower half** — a `golang:*-alpine` pull from Docker Hub can crawl at a few hundred KB/s from China, minutes of nothing, while an `apk add` through the aliyun mirror finishes in under a second. Mirroring only the packages fixes the visible half and leaves the wait.

Two ways, not exclusive:

- **Configure the daemon** (`/etc/docker/daemon.json`, then restart Docker) — applies to every image, changes no Dockerfile:
  ```json
  { "registry-mirrors": ["https://docker.m.daocloud.io", "https://dockerproxy.com"] }
  ```
  This is the better default, but it is machine state: it cannot be committed, so a fresh CI runner or a new laptop is slow again until someone remembers.
- **Make the registry an `ARG`** (as the skeleton does) so the Dockerfile itself can point elsewhere — `docker build --build-arg BASE_REGISTRY=registry.cn-hangzhou.aliyuncs.com ...`. Committed and reproducible, but each mirror namespaces images differently, so verify the path resolves rather than assuming `library/` works everywhere.

State which one the project relies on. "It builds on my machine in 40 seconds" and "it builds in CI in 40 minutes" is the same Dockerfile with different daemon config, and that difference is invisible in the repo.
...
- **`HEALTHCHECK`: usually leave it out, deliberately.** Under an orchestrator, k8s `liveness`/`readiness` probes own this and a Dockerfile `HEALTHCHECK` is dead weight that k8s ignores outright. Say so in a comment rather than leaving a blank — the silence is what gets filled in with the wrong thing. The wrong thing is specific: `HEALTHCHECK CMD curl -f http://localhost:8080/healthz` works on `alpine` and **breaks the moment the runtime moves to `scratch`/distroless**, where there is no `curl`, no `wget`, and no shell — and it breaks as a permanently-unhealthy container, not a build error. If you do need one in the image (plain `docker run`, compose-managed services, a swarm), make the *binary itself* the probe — `HEALTHCHECK CMD ["/app/app", "healthcheck"]` — a subcommand that hits its own endpoint and exits 0/1. That has no runtime dependency and cannot rot when the base image shrinks.
