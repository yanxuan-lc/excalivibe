...
"${CLAUDE_PLUGIN_ROOT}/skills/notify-user/scripts/notify.sh" --doctor
```
...
```bash
"${CLAUDE_PLUGIN_ROOT}/skills/notify-user/scripts/notify.sh" --init     # writes the config, both lines live
"${CLAUDE_PLUGIN_ROOT}/skills/notify-user/scripts/notify.sh" --doctor   # prove it, with a real banner
```
