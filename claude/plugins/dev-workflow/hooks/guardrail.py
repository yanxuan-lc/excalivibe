#!/usr/bin/env python3
"""
dev-workflow — PreToolUse irreversible-action guardrail.

A deterministic backstop for the flow's "an irreversible act needs a human" boundary. It is
NOT the flow's enforcement — the delivery stage puts those acts on `effect` steps run by the
main agent and on `genai.deliver`, which is `protocol: human` and cannot pass without a
recorded decision. This is a second, mechanical line of defence for when the flow is
bypassed, an agent goes off-script, or a run is unattended.
