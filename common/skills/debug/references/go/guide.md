...
Check for Delve first, and install it only if the environment allows:

```bash
command -v dlv || go install github.com/go-delve/delve/cmd/dlv@latest
```

**If the install is not available to you, Loop C is not available either — fall back to Loop A.**
That install writes to `$GOPATH/bin` and pulls from the network, which a CI runner, a sandbox, a
locked-down corporate machine, or an evaluation harness may all refuse. This is far more common
than the no-TTY case the skill already covers, and the fallback is the same: a tagged log-loop
reaches the same variable values, just with more round-trips. Say in your report that Loop C was
unavailable and why — "no debugger here" is a fact about the environment worth recording, not a
gap to work around silently.
