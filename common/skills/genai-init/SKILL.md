...
```bash
test -f docs/flow-scratch-skill.md || echo "MISSING  flow-scratch skill — the driving manual"
```
...
```bash
fsx skill install --target generic
openspec init --tools agents --no-animation     # after step 1 confirms the repository root
```
