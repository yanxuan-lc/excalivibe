...
- **Middleware only.** Orchestrate the stateful dependencies a dev needs locally — database (MySQL/PostgreSQL), message queue (Kafka/RabbitMQ/NSQ), cache (Redis), object store (MinIO). **Do not put your own service in compose.** The service runs on the host (`make server-run`) or in a debugger, so you can rebuild/restart/attach instantly without a compose round-trip. Compose owns the things you *don't* iterate on.
  ...
  **When the ask is "one command to bring everything up", give them that — through Make, not through compose.** The request is reasonable and refusing it is not an answer; what people want is a single command, not their service inside compose specifically. A prerequisite chain delivers it and keeps the service on the host:

  ```makefile
  server-run: deps-up server-build   ## bring up middleware, build, and run the service
  	@. $(DEPS_ENV); exec ./bin/server
  ```

  One command, dependencies explicit, and the binary still rebuilds in seconds. If they *also* need the shipped image exercised against those dependencies, that is a separate target joining the compose network by service name — not a compose entry:

  ```makefile
  server-docker-run: deps-up        ## run the built image against the compose middleware
  	docker run --rm --network $(COMPOSE_NET) -e APP_DB_HOST=mysql $(IMAGE)
  ```

  Note the host differs: `127.0.0.1:$(DEV_MYSQL_PORT)` from the host, the service name `mysql` from inside the network. A containerised service cannot reach the host's mapped port.
...
The application's DSN is composed from the same `DEV_MYSQL_PORT` (e.g. in `.env`: `APP_DB_DSN=app:app@tcp(127.0.0.1:${DEV_MYSQL_PORT})/app?...`), so service and container always agree. That nested reference resolves because Make's `include` creates *recursively-expanded* variables — the value is evaluated when used, not when included, so `tmp/deps.env` and `.env` can be included in either order and the port still lands. (make.md's "no shell runs on them" is about shell metacharacters; Make's own `${…}` expansion still applies. Verified on GNU Make 3.81, the version macOS ships.)

Notes:
- **`-include $(DEPS_ENV)` happens at parse time; `deps-up` writes it at recipe time.** On the very first run the file does not exist yet, so `$(DEV_MYSQL_PORT)` is empty for the *whole* make process — including the rest of this recipe. That is why the lines after allocation re-read the file with `. $(DEPS_ENV)` instead of using `$(DEV_MYSQL_PORT)`; it is not redundant, and dropping it breaks only the first run, which is the run nobody re-tests.
- **Some middleware needs a *pair* of ports with a fixed offset**, not two independent ones. Nacos is the common case: the Go SDK derives its gRPC port as `port + 1000` rather than reading a second setting, so a host mapping that allocates the two independently leaves the client authenticating over HTTP on the mapped port and then dialling an unmapped one. The symptom is config reads hanging until timeout, with an error that never mentions ports. Allocate a base port whose `base + offset` is also free, and map both.
- **Allocating N ports amplifies the detect→bind race N times.** The single-port window below is small enough to live with; at two or more, prefer the race-free form (`"0:3306"` plus `docker compose port`, described at the end of these notes).
