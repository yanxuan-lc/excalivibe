...
The routing table sends plain-JS projects here "minus the type-level advice". Concretely:

- **The gate is `npx vitest run` + ESLint. `tsc --noEmit` is dropped**, and dropping it is not a
  weakened gate — there are no types to check. Do not install TypeScript to satisfy a step that
  has nothing to verify.
- **Config is `vitest.config.mjs`**, and the `fileURLToPath(new URL(...))` alias above is what
  you want — `__dirname` is undefined under `"type": "module"`, which is the normal setup here.
- **Test files are `<module>.test.mjs`** under `tests/`, mirroring `src/` — same layout as
  TypeScript, the extension follows `coding-guideline`'s naming table.
- Everything else on this page — the runner, the coverage gate, the mocking tools, the file
  organization — applies unchanged.

## Project Setup

```bash
npm install -D vitest @vitest/coverage-v8 typescript @types/node
```

`typescript` and `@types/node` are not optional here: the gate pass runs `tsc --noEmit`, and the
config below imports `node:*`. Leaving them out gets you a green Vitest run and a gate that
cannot execute.
...
export default defineConfig({
  resolve: {
    alias: {
      // NOT __dirname: it does not exist in an ESM config ("type": "module"), and
      // referencing it fails at config load, before a single test runs.
