---
name: a11y-check
description: Audit a UI surface for accessibility problems with axe or Lighthouse, map each finding to a WCAG success criterion, and return a pass/fail verdict against a budget — together with the criteria automation cannot check, listed as open items rather than silently passed. A green scanner run is not an accessible page, and saying so is part of the job. Use whenever a user-facing surface needs checking, on "is this accessible", "run an a11y audit", "check the contrast", "WCAG compliance", "does this work with a screen reader", or before shipping UI. Read-only toward the application — it reports and never fixes.
description-claude: Audit a UI surface for accessibility problems with axe or Lighthouse, map each finding to a WCAG success criterion, and return a pass/fail verdict against a budget — together with the criteria automation cannot check, listed as open items rather than silently passed. A green scanner run is not an accessible page, and saying so is part of the job. Use whenever a user-facing surface needs checking, on "is this accessible", "run an a11y audit", "check the contrast", "WCAG compliance", "does this work with a screen reader", or before shipping UI. Read-only toward the application — it reports and never fixes.
description-codex: Audit a UI surface for accessibility problems with axe or Lighthouse, map each finding to a WCAG success criterion, and return a pass/fail verdict against a budget — together with the criteria automation cannot check, listed as open items rather than silently passed. A green scanner run is not an accessible page, and saying so is part of the job. Use whenever a user-facing surface needs checking, on "is this accessible", "run an a11y audit", "check the contrast", "WCAG compliance", "does this work with a screen reader", or before shipping UI. Read-only toward the application — it reports and never fixes.
---
...
<!--@claude-->
   the browser through the `graceful-browser` skill (it picks the best available browser
