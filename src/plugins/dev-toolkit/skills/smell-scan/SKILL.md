---
name: smell-scan
description: Sweep a codebase or a target slice for architecture and code bad smells and emit a ranked report of refactoring candidates, each with a verdict and a recommendation strength, so cleanup gets picked by evidence rather than by whoever complains loudest. Use on "what should we refactor", "this module has gotten messy", "find the tech debt", "where is the duplication", "code smells", or when deciding what to clean up next. Detection only — it never edits code, never performs a refactor, and never decides which candidates get applied.
description-claude: Sweep a codebase or a target slice for architecture and code bad smells and emit a ranked report of refactoring candidates, each with a verdict and a recommendation strength, so cleanup gets picked by evidence rather than by whoever complains loudest. Use on "what should we refactor", "this module has gotten messy", "find the tech debt", "where is the duplication", "code smells", or when deciding what to clean up next. Detection only — it never edits code, never performs a refactor, and never decides which candidates get applied.
description-codex: Sweep a codebase or a target slice for architecture and code bad smells and emit a ranked report of refactoring candidates, each with a verdict and a recommendation strength, so cleanup gets picked by evidence rather than by whoever complains loudest. Use on "what should we refactor", "this module has gotten messy", "find the tech debt", "where is the duplication", "code smells", or when deciding what to clean up next. Detection only — it never edits code, never performs a refactor, and never decides which candidates get applied.
---
...
Write a ranked markdown report named **`smell-report.md`** (or `refactor-candidates.md`
when the caller asks for the candidates-only view). Lowercase-kebab is the project's
artifact-naming rule — UPPERCASE is reserved for the closed set of documents a human opens
on purpose, and a scan report is not one. One section per candidate, strongest first:
