---
name: security-scan
description: Run three classes of static security check over a change or a whole codebase — SAST for code-level vulnerability patterns, a dependency audit for known CVEs, and secret scanning for hardcoded credentials — and report every finding ranked by severity. Use on "check this for security issues", "any vulnerable dependencies", "did we leak a key", "scan before release", or whenever a change touches authentication, input handling, crypto, file paths, or third-party packages. Note that scanning only a diff misses secrets committed earlier, so the scope is part of the answer. It reports findings and leaves the blocking decision to whoever owns the threshold.
description-claude: Run three classes of static security check over a change or a whole codebase — SAST for code-level vulnerability patterns, a dependency audit for known CVEs, and secret scanning for hardcoded credentials — and report every finding ranked by severity. Use on "check this for security issues", "any vulnerable dependencies", "did we leak a key", "scan before release", or whenever a change touches authentication, input handling, crypto, file paths, or third-party packages. Note that scanning only a diff misses secrets committed earlier, so the scope is part of the answer. It reports findings and leaves the blocking decision to whoever owns the threshold.
description-codex: Run three classes of static security check over a change or a whole codebase — SAST for code-level vulnerability patterns, a dependency audit for known CVEs, and secret scanning for hardcoded credentials — and report every finding ranked by severity. Use on "check this for security issues", "any vulnerable dependencies", "did we leak a key", "scan before release", or whenever a change touches authentication, input handling, crypto, file paths, or third-party packages. Note that scanning only a diff misses secrets committed earlier, so the scope is part of the answer. It reports findings and leaves the blocking decision to whoever owns the threshold.
---
...
Write the report to **`genai/security-report.md`** — that exact name, because the `security`
node's predicate reads it. (This skill used to say `security-scan-report.md`, which the gate
never looked at.) It must let a caller threshold on it mechanically:

- **A verdict anchor**, which is what the gate actually reads:

  ```markdown
  <!-- genai:security.verdict blocking-open=0 -->
  ```

  `blocking-open` is the count of **Critical + High** findings still unresolved. The predicate
  used to scan for the words `BLOCKING` or `P0`, neither of which this skill's taxonomy ever
  produces — so that gate was **structurally incapable of failing on a Critical**. A declared
  count fixes that without forcing the report into someone else's severity vocabulary.
