---
name: vcs-workflow
description: Decide how to branch, version and ship work in this project, and apply the conventions instead of improvising — feature branches in worktrees, merging into an integration branch, merge requests into a protected main, and the release sequence of bump, merge, build, publish, tag. The version call is the single most common question here, so reach for it on "is this a patch, a minor or a major", "what number should this release be", and when version strings across manifests disagree and you need to know which is authoritative. Also for "where should this work live", "how does this reach main", and where a submodule pointer should be moved to in a meta-repository. Publishing is irreversible, so it separates the reversible steps from the ones that are not. It answers how work should be structured and shipped, not how to carry out an individual git command whose intent is already clear.
description-claude: Decide how to branch, version and ship work in this project, and apply the conventions instead of improvising — feature branches in worktrees, merging into an integration branch, merge requests into a protected main, and the release sequence of bump, merge, build, publish, tag. The version call is the single most common question here, so reach for it on "is this a patch, a minor or a major", "what number should this release be", and when version strings across manifests disagree and you need to know which is authoritative. Also for "where should this work live", "how does this reach main", and where a submodule pointer should be moved to in a meta-repository. Publishing is irreversible, so it separates the reversible steps from the ones that are not. It answers how work should be structured and shipped, not how to carry out an individual git command whose intent is already clear.
description-codex: Decide how to branch, version and ship work in this project, and apply the conventions instead of improvising — feature branches in worktrees, merging into an integration branch, merge requests into a protected main, and the release sequence of bump, merge, build, publish, tag. The version call is the single most common question here, so reach for it on "is this a patch, a minor or a major", "what number should this release be", and when version strings across manifests disagree and you need to know which is authoritative. Also for "where should this work live", "how does this reach main", and where a submodule pointer should be moved to in a meta-repository. Publishing is irreversible, so it separates the reversible steps from the ones that are not. It answers how work should be structured and shipped, not how to carry out an individual git command whose intent is already clear.
---
...
git fetch --prune
git branch -d feat/<name>                 # only AFTER the push has happened — see below
```

**Deleting the branch has to wait for the push.** `git branch -d` measures "merged" against the
**upstream** branch, not against your local `dev`, so before `git push origin dev` it refuses:

```
warning: not deleting branch 'feat/X' that is not yet merged to 'refs/remotes/origin/dev',
         even though it is merged to HEAD
error: the branch 'feat/X' is not fully merged
```

Do **not** reach for `-D`. That discards git's only check at the one moment the work exists in
exactly one place. Since the push waits for the user, so does the branch deletion — leave the
branch, say so, and delete it after they push. `git worktree remove` is safe to do immediately;
it is the branch that has to wait.
...
- **Merging work into the integration branch with its checks still open.** Whatever this project treats as the bar for merging — review comments resolved, tests green on disk rather than assumed, acceptance actually run — has to hold before the merge, not after. A merge that skips it ships unverified state into the branch everyone else builds on, and the cost lands on whoever pulls next rather than on whoever skipped it.
...
git worktree remove ../proj-feat-X         # safe now
git branch -d feat/X                       # only after the push — -d checks against origin/dev
