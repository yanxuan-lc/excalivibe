...
> **The canonical signature is `func FetchUser(ctx context.Context, id string) (User, error)`.**
> Snippets below sometimes drop `ctx` to keep the error-handling point in focus — that is
> shorthand, not a second convention. Anything crossing a boundary (HTTP, DB, RPC) takes
...
func fetchUser(id string) (User, error) {
    // Never concatenate caller input into a URL: url.PathEscape keeps `id` a single
    // opaque segment, so "../../admin" can't climb out of /users/ (see Validate at Boundaries).
    endpoint, err := url.JoinPath(userURL, url.PathEscape(id))
    if err != nil {
        return User{}, fmt.Errorf("building user URL for %s: %w", id, err)
    }
    ...
    // A non-2xx response is an expected failure path, so it is an error — never a panic.
    if resp.StatusCode != http.StatusOK {
        return User{}, fmt.Errorf("fetching user %s: unexpected status %s", id, resp.Status)
    }
    // ...
}
```
...
1. DDL comment on the column (`COMMENT 'event time, Beijing time +08:00'`).
