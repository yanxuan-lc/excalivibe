...
cc := *constant.NewClientConfig(
    ...
    constant.WithDisableUseSnapShot(true),     // ← the one that actually does it: no local
                                               //   snapshot on remote failure, at any time
    constant.WithNotLoadCacheAtStart(true),    // no cache preload during construction
...
**`WithDisableUseSnapShot(true)` is not optional, and the other two do not substitute for it.**
Their names read as though they cover this, and they do not: `NotLoadCacheAtStart` turns off the
preload that happens while the client is being constructed, and `UpdateCacheWhenEmpty` is about
empty *service-instance* results. Neither touches what `GetConfig` does when the server call
fails — which is to fall back to the on-disk snapshot. That is why the SDK's own error text for
a total miss reads `read config from both server and cache fail`: **both** have to miss before
you hear about it.

Left out, the failure is the worst-shaped one there is. With a snapshot in `CacheDir` and Nacos
unreachable, the service starts normally, `/readyz` returns ok, and the log says the config was
loaded from Nacos — while it is running on whatever was cached, possibly months old. Nothing
alerts, because from the outside it looks like a healthy boot. With the option set, the same
scenario exits non-zero with `get config from remote nacos server fail, and is not allowed to
read local file`.

Verify it rather than trusting the flag: put a sentinel value in the cache file
(`<CacheDir>/config/<DATA_ID>@@<GROUP>@@<NAMESPACE>`), point the client at a dead address, and
confirm the process exits non-zero instead of booting on the sentinel.
