...
Remove every tagged line, then verify with `grep -rn '\[debug:' .` — result must be empty. (Not
`git diff | grep`: a removed probe is a `-` line that still contains the marker.)
