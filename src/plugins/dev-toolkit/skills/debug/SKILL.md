---
name: debug
description: Find the cause of a failure whose cause is not yet obvious, by picking the investigation loop that fits the symptom and running it to falsification instead of reading code until something looks wrong. Carries three distinct loops — the log-loop for output that is wrong, exit-code convergence for a process that dies, CLI-debugger attachment for state at a moment — with the per-stack invocations for each and the cleanup discipline that keeps instrumentation from being left behind. Which loop applies is decided by the symptom, so reach for it before starting to investigate rather than after — a crash, a stack trace, a failing or flaky test, output that is wrong while the tests stay green, a hang, "it only fails in CI", "this worked yesterday". It locates a cause; carrying out a fix whose cause is already established is separate work.
description-claude: Find the cause of a failure whose cause is not yet obvious, by picking the investigation loop that fits the symptom and running it to falsification instead of reading code until something looks wrong. Carries three distinct loops — the log-loop for output that is wrong, exit-code convergence for a process that dies, CLI-debugger attachment for state at a moment — with the per-stack invocations for each and the cleanup discipline that keeps instrumentation from being left behind. Which loop applies is decided by the symptom, so reach for it before starting to investigate rather than after — a crash, a stack trace, a failing or flaky test, output that is wrong while the tests stay green, a hang, "it only fails in CI", "this worked yesterday". It locates a cause; carrying out a fix whose cause is already established is separate work.
description-codex: Find the cause of a failure whose cause is not yet obvious, by picking the investigation loop that fits the symptom and running it to falsification instead of reading code until something looks wrong. Carries three distinct loops — the log-loop for output that is wrong, exit-code convergence for a process that dies, CLI-debugger attachment for state at a moment — with the per-stack invocations for each and the cleanup discipline that keeps instrumentation from being left behind. Which loop applies is decided by the symptom, so reach for it before starting to investigate rather than after — a crash, a stack trace, a failing or flaky test, output that is wrong while the tests stay green, a hang, "it only fails in CI", "this worked yesterday". It locates a cause; carrying out a fix whose cause is already established is separate work.
---
...
>
> **With one exception: a test *you* wrote to reproduce the bug does not count.** Phase 0 tells
> you to manufacture a failing test when none exists, and that test then satisfies row 2 and
> routes you into Loop B — where the traceback points at your own `assert` and tells you nothing
> about the defect. Loop B converges on a signal the *code under investigation* emitted: a stack
> trace from its own frames, a compiler error, a non-zero exit it produced. A red assertion you
> authored is a reproduction harness, not a diagnosis; keep it as the pass/fail oracle and run
> **Loop A** against it.
...
4. **Decide which side is wrong before touching either.** A red assertion says the code and the
   test disagree; it does not say which one is right. Answer that from the **specification** —
   the spec, the doc comment, the API contract, the ticket — never from which side is easier to
   change. Then say which it is:
   - **product bug** — the code violates the stated behaviour. Continue to step 5.
   - **test bug** — the test asserts something never promised (stale fixture, wrong expected
     value, a changed contract nobody updated). **Fix the test, and say that you did.** Silently
     editing the test to green is the one move that destroys the value of the whole suite.
   - **the specification is silent** — neither side is wrong yet, because nothing was decided.
     Stop and surface it. Picking one and shipping it makes an unmade product decision
...
6. **Rerun the command.** If exit code is 0 and all targets pass, the loop is complete. If non-zero, read the new output and repeat from step 3 with the updated information.
...
**Mandatory after any loop that added tagged probes** — and the self-check below is mandatory
after *every* session, including one that added none. Running it on a probe-free session costs a
single command and confirms what you believe; skipping it is how a session that "obviously added
nothing" leaves a probe behind. (The Guardrails phrase this unconditionally; this is the same
rule, said once here.)
...
4. Run the self-check:
   ...
   ```bash
   grep -rn '\[debug:' .        # must print nothing
   ```

   This is the authoritative check — the question is whether a marker survives **in the working
   tree**, so ask the working tree. Any hit means a probe is still there; remove it and re-run.

   If you also want to inspect the diff, look at **added** lines only:

   ```bash
   git diff | grep '^+.*\[debug:'
   ```

   Plain `git diff | grep '\[debug:'` is **wrong and will loop forever**: a probe that was ever
   committed shows up as a deleted `-` line still containing the marker, so a perfectly clean
   removal reports "not clean" and the procedure tells you to remove it again.
