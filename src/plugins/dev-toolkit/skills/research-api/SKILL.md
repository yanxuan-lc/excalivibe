---
name: research-api
description: Discover an API's OpenAPI or Swagger spec and probe its endpoints with real calls, read-only first, so an answer about an interface rests on observed responses rather than on documentation that may be stale or aspirational. Use whenever a question depends on how an API actually behaves — "what does this endpoint return", "is that field ever null", "which auth does it want", "how does its pagination work", "why is this call 422" — or before writing an integration against a service you have never called.
description-claude: Discover an API's OpenAPI or Swagger spec and probe its endpoints with real calls, read-only first, so an answer about an interface rests on observed responses rather than on documentation that may be stale or aspirational. Use whenever a question depends on how an API actually behaves — "what does this endpoint return", "is that field ever null", "which auth does it want", "how does its pagination work", "why is this call 422" — or before writing an integration against a service you have never called.
description-codex: Discover an API's OpenAPI or Swagger spec and probe its endpoints with real calls, read-only first, so an answer about an interface rests on observed responses rather than on documentation that may be stale or aspirational. Use whenever a question depends on how an API actually behaves — "what does this endpoint return", "is that field ever null", "which auth does it want", "how does its pagination work", "why is this call 422" — or before writing an integration against a service you have never called.
---
