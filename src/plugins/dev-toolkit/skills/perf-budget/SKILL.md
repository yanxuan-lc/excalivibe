---
name: perf-budget
description: Measure a change against performance budgets and report every affected dimension as measured, threshold, and baseline plus delta, with a pass/fail per dimension — latency, query count and N+1, bundle size, and whatever else the project budgets. A bare number is not a verdict; three numbers are. Use whenever a change might have cost something at runtime, on "did this get slower", "check the bundle size", "is this an N+1", "performance regression", or before shipping something latency-sensitive. It produces the verdict and does not enforce it.
description-claude: Measure a change against performance budgets and report every affected dimension as measured, threshold, and baseline plus delta, with a pass/fail per dimension — latency, query count and N+1, bundle size, and whatever else the project budgets. A bare number is not a verdict; three numbers are. Use whenever a change might have cost something at runtime, on "did this get slower", "check the bundle size", "is this an N+1", "performance regression", or before shipping something latency-sensitive. It produces the verdict and does not enforce it.
description-codex: Measure a change against performance budgets and report every affected dimension as measured, threshold, and baseline plus delta, with a pass/fail per dimension — latency, query count and N+1, bundle size, and whatever else the project budgets. A bare number is not a verdict; three numbers are. Use whenever a change might have cost something at runtime, on "did this get slower", "check the bundle size", "is this an N+1", "performance regression", or before shipping something latency-sensitive. It produces the verdict and does not enforce it.
---
...
<!--@claude-->
  page-load timings by **driving a browser (via the `graceful-browser` skill)** and
...
Write the report to **`genai/perf-report.md`** — that exact name; the `perf` node's predicate
reads it. Open it with the verdict anchor:

```markdown
<!-- genai:perf.verdict blocking-open=0 -->
```

`blocking-open` is the count of dimensions **over budget**. The node used to check only that the
file was non-empty, so a report whose own verdict read `FAIL — 5000ms against a 200ms budget`
passed the gate exactly like a clean one. A budget nobody can fail is not a budget.

Then the three numbers per dimension (measured / threshold / baseline) as described above. State
where each budget came from; a threshold with no provenance is a number someone made up.
