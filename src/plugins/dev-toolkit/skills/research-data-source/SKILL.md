...
description: Connect read-only to a real data source and inspect its schema, sample rows, value distributions and volume, so an answer about data rests on what is actually stored rather than on what the schema implies. Use whenever a question depends on real data — "what values does this column actually hold", "how many rows are we talking about", "is this field ever null in practice", "what does that join really produce" — or before designing against a table nobody has looked inside.
description-claude: Connect read-only to a real data source and inspect its schema, sample rows, value distributions and volume, so an answer about data rests on what is actually stored rather than on what the schema implies. Use whenever a question depends on real data — "what values does this column actually hold", "how many rows are we talking about", "is this field ever null in practice", "what does that join really produce" — or before designing against a table nobody has looked inside.
description-codex: Connect read-only to a real data source and inspect its schema, sample rows, value distributions and volume, so an answer about data rests on what is actually stored rather than on what the schema implies. Use whenever a question depends on real data — "what values does this column actually hold", "how many rows are we talking about", "is this field ever null in practice", "what does that join really produce" — or before designing against a table nobody has looked inside.
---
...
<!--@codex-->
1. **A connected MCP database/warehouse tool** — zero install, access already scoped. Best choice when present (check whether the session exposes an `mcp__<server>__*` database tool).
<!--@end-->
...
- The query you ran (or its shape) and the sampling/aggregation basis (e.g. "100-row sample", "full `COUNT` as of <time>").
