...
description: Clone a library at an exact tag, branch or commit and read its real source, recording SHA-level provenance so every claim traces back to a specific line at a specific revision. Use whenever a question depends on what a dependency actually does rather than what its README says — "how does this library handle that case", "is this a bug in the package or in our code", "what changed between these two versions", "does it really retry" — and whenever an answer would otherwise rest on recalled or inferred behavior.
description-claude: Clone a library at an exact tag, branch or commit and read its real source, recording SHA-level provenance so every claim traces back to a specific line at a specific revision. Use whenever a question depends on what a dependency actually does rather than what its README says — "how does this library handle that case", "is this a bug in the package or in our code", "what changed between these two versions", "does it really retry" — and whenever an answer would otherwise rest on recalled or inferred behavior.
description-codex: Clone a library at an exact tag, branch or commit and read its real source, recording SHA-level provenance so every claim traces back to a specific line at a specific revision. Use whenever a question depends on what a dependency actually does rather than what its README says — "how does this library handle that case", "is this a bug in the package or in our code", "what changed between these two versions", "does it really retry" — and whenever an answer would otherwise rest on recalled or inferred behavior.
---
...
Write conclusions into whatever report this investigation feeds — a research write-up, an issue comment, the answer itself. Wherever it lands, every source-backed claim carries its provenance:
